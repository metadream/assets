  ...
  constructor(target) {
    ...
    this.$target.on('click', () => {
      if (this.isOpen) {
        ...
        this.$wrapper.style.top = pos.y + pos.height + scrollTop + 2 + 'px';
...
Quick.FlexTable = class {
  ...
  bindEvents() {
    for (const row of this.$table.rows) {
      const icons = row.querySelectorAll('td:first-child>div');
      icons[1].onclick = () => this.addRow(row);

      const deletable = row.getAttribute('deletable');
      if (deletable == '' || deletable == 'true') {
        icons[0].style.visibility = 'visible';
        icons[0].onclick = () => this.removeRow(row);
      }
    }
  }
  ...
  createPlusIcon() {
    return this.createIcon('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1024 1024" width="20" height="20"><path fill="var(--primary-color)" d="M978 162c0-64-52-116-116-116H162C98 46 46 98 46 162v700c0 64 52 116 116 116h700c64 0 116-52 116-116zM768 563H563v205H461V563H256V461h205V256h102v205h205z"/></svg>');
  }

  createMinusIcon() {
    return this.createIcon('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1024 1024" width="20" height="20"><path fill="#999" d="M978 162c0-64-52-116-116-116H162C98 46 46 98 46 162v700c0 64 52 116 116 116h700c64 0 116-52 116-116zM768 563H256V461h512z"/></svg>');
  }
