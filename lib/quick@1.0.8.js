 ...
 * @param options { maxCount, maxSize, mimeTypes, onUploaded } 配置项
 * - maxCount {number} 最大允许数量
...
Quick.Attachment = class {
  ...
  constructor(selector, list, options) {
    ...
    if (this.editable) {
      ...
      this.$wrapper.appendChild($(`<div><input type="file" accept="${mimeTypes}" ${this.maxCount>1?"multiple":""}/><a>上传附件</a></div>`));
  ...
  upload(e) {
    ...
    if (files.length > this.maxCount) {
      return Quick.error(`一次最多上传${this.maxCount}个附件`);
    }
    ...
    Quick.http.post(Quick.ATTACHMENT_API, fd).then(list => {
      ...
      if (list) {
        Quick.success('上传成功');
        this.build(list);
        this.onUploaded && this.onUploaded(list);
      }
    });
  }
...
Quick.form = {
  ...
  validator: {
    ...
    varchar: function(value, min, max) {
      ...
      const pattern = new RegExp('^[\\s\\S]{' + min + ',' + max + '}$');
